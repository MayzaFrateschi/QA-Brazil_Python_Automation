...
class UrbanRoutesPage:
    ...
    comfort_active = (
        By.XPATH,
        '//*[@id="root"]/div/div[3]/div[3]/div[2]/div[1]/div[5]'
    )
    ...
    switch_blanket_active = (By.CSS_SELECTOR, ".switch input")
    ...
    def confirm_cartao(self):
        # retorna o texto do cartão selecionado/confirmado
        el = WebDriverWait(self.driver, timeout=10).until(
            EC.visibility_of_element_located(self.confirm_card)
        )
        return el.text

    def add_comentario(self, mensagem):
        campo = WebDriverWait(self.driver, timeout=10).until(
    ...
    def coment_confirm(self):
        # valida pelo valor do campo (mais confiável em inputs)
        campo = WebDriverWait(self.driver, timeout=10).until(
            EC.visibility_of_element_located(self.add_comment)
        )
    ...
    def switch_cobertor_active(self):
        checkbox = WebDriverWait(self.driver, timeout=10).until(
